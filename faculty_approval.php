...
<body>
    <nav>
        ...
        <div class="texts-container">
            ...
            <p style="color: white;">faculty Clearance approval process</p>
        </div>
    ...
    <div class="main-container">
        <div class="left-side-container">
          <div class="approval-details-container">
            <div class="detail">
                <facultyel for="st_name">Student name:</facultyel>
                <p><?php echo $fullName; ?></p>
            </div>
            <div class="detail">
                <facultyel for="id_num">RegNo:</facultyel>
                <p> <?php echo $manage_id; ?> </p>
            </div>
            <div class="detail">
                <facultyel for="id_num">Request Reason:</facultyel>
                <p> <?php echo $reason; ?> </p>
            </div>
...
<!-- PROPERTY MANAGEMENT -->
            ...
            <?php
              $msg="";
              $status_msg="";
                $sql="SELECT * FROM faculty_record WHERE student_id='$manage_id'";
                ...
                ?>
                <facultyel for="id_num">Property taken:</facultyel>
                <p><?php echo $msg; ?> &radic;</p>
            </div>
            <div class="detail allApprove">
                <facultyel for="id_num">Draw Signature:</facultyel>
                ...
                <?php
                  ...
                  $reg_query=mysqli_query($conn,"SELECT * FROM faculty_final_approve_reject WHERE student_id='$manage_id' ") or die(mysqli_error($conn));
                  $rows=mysqli_num_rows($reg_query);
                  if($rows>0){
                    echo "<p style=color:red;>Already approved...!!</p>"; 
                  }else{
                    $date = date('Y-m-d H:i:s');
                  $sql = mysqli_query($conn,"INSERT INTO faculty_final_approve_reject (student_id,status,signature,staff_name,date) VALUES('$manage_id','$status_msg','$file_name','".$_SESSION['faculty_full_name']."','$date')") or die(mysqli_error($conn));
                    $sql_delete_request=mysqli_query($conn,"DELETE FROM faculty_request WHERE id='$manage_id'") or die(mysqli_error($conn));
                    $sql_delete_request=mysqli_query($conn,"DELETE FROM faculty_rejected_requests WHERE student_id='$manage_id'") or die(mysqli_error($conn));
                    echo "<h4 style=color:green;margin-top:20px;>Clearance approved successfully...</p>";
                    $_SESSION['faculty_approved']=$status_msg;
                  }
                }
                  ?>
                  ...
                  $status_msg="Rejected";
                  $sql = mysqli_query($conn,"INSERT INTO faculty_rejected_requests (student_id,status,staff_username) VALUES('$manage_id','$status_msg','".$_SESSION['faculty_full_name']."')") or die(mysqli_error($conn));
