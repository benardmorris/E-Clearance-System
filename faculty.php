                   ...
                   <li>
                    <span class="icon"> <img src="icons/manage(copy).png" width="20px" height="20px" alt=""> </span>
                     ...
                     <div class="dropdown-container" id="dropdown-cont" style="flex-direction:column;">
                        <a href="#material-register" onMouseOver="this.style.color='green'" onMouseOut="this.style.color='white'">Record faculty Material</a>
      ...
      <div class="main-home">
        <div class="top-container">
            ...
            <div class="top-title" style="display: flex; flex-direction: column; align-items:center; justify-content:center;">
                ...
                <p><strong>Students online clearance system-FACULTY</strong></p>
            </div>
            <div class="profile-cont">
            <img src="<?php echo $_SESSION['faculty_image']; ?>" alt="" width="60px" height="60px">
                <div class="dropdown">
                      <button onclick="myFunction()" class="dropbtn"><?php echo $_SESSION ['faculty_u_name'];?>&#9660;</button>
          ...
          <a name="dashboard">
            <div class="dashboard-container">
              <div class="dashboard">
                ...
                <?php
                  $stmt="SELECT material_id FROM faculty_record ORDER BY material_id";
              ...
              <div class="dashboard">
                ...
                <?php
                  $stmt="SELECT id FROM faculty_request ORDER BY id";
                  $res=mysqli_query($conn,$stmt) or die(mysqli_error($conn));
                  $row=mysqli_num_rows($res);
                  ?>
                  <p><?php echo $row ?></p>
              ...
              <div class="dashboard">
                ...
                <?php
                  $stmt="SELECT student_id FROM faculty_final_approve_reject ORDER BY student_id";
                  $res=mysqli_query($conn,$stmt) or die(mysqli_error($conn));
                  $row=mysqli_num_rows($res);
                  ?>
                  <p> <?php echo $row; ?> </p>
              ...
              <div class="dashboard">
                ...
                <?php
                  $stmt="SELECT student_id FROM faculty_rejected_requests ORDER BY student_id";
                  $res=mysqli_query($conn,$stmt) or die(mysqli_error($conn));
                  $row=mysqli_num_rows($res);
                  ?>
                  <p> <?php echo $row; ?> </p>
...
<!-- VIEW REQUEST CONTAINER -->
           ...
           <div class="view-request-container">
             ...
             <?php
              $sql="SELECT * FROM faculty_request INNER JOIN student ON faculty_request.id = student.id";
              ...
              <div class="requests">
                ...
                <button>
                  <a href="faculty_approval.php?manageid=<?php echo $student_id; ?>&fullName=<?php echo $fullName; ?>&reason=<?php echo $reason; ?>">Manage</a>
                </button>
             </div>
             <hr>
             <?php
           ...
           <div class="material-lend-register">
               ...
               <div class="form-row">
                 ...
                 <div class="form-row">
                   ...
                   <input type="text" name="given_by" value="<?php echo $_SESSION['faculty_full_name']; ?>" required>
                 </div>
                 ...
                 <?php
                     if(isset($_POST["register"]))
                     {
                       ...
                       $given_by=$_POST["given_by"];
                               $sql2=mysqli_query($conn,"INSERT INTO faculty_record(material_id,material_name, date_taken, taken_by,student_id,given_by)
...
<!--MANAGE PROPERTY CONTAINER-->
                        ...
                        <div class="edit-delete-material-lend-container">
                         <div class="top-h1-add-container">
                           <h3 style="margin: 10px;">Lists of Material lend </h3>
                            ...
                            $query=mysqli_query($conn,"SELECT * FROM faculty_record WHERE CONCAT(material_name,student_id,taken_by) LIKE '%$filterValues%'") or die(mysqli_error($conn));
                            ...
                            if($rows_srch>0){
                              while ($rows_srch=mysqli_fetch_assoc($query)){
                                ...
                                $table_name="faculty_record";
                                $header="faculty.php#edit-delete-lend";
                                ?>
                                <tr>
                   ...
                   <?php
                         ...
                         <?php
                          ...
                          ?>
                       <?php
                        $sql="SELECT * FROM faculty_record ";
                        $res=mysqli_query($conn,$sql) or die( mysqli_error($conn));
                        $rows=mysqli_num_rows($res);
                        if($rows>0){
                      while ($rows=mysqli_fetch_assoc($res)){
    ...
    <div class="popup-profile-container" id="popup-profile" >
      <div class="profile-container">
       <div class="profile-detail">
        ...
        <p  id="firstnm"><?php echo $_SESSION['faculty_full_name']; ?></p>
        </div>
        <hr>
        <div class="detail" >
          <label  for="faculty">ID :</label>
          <p  id="faculty"><?php echo $_SESSION['faculty_id']; ?></p>
        </div>
        <hr>
        <div class="detail" >
          <label  for="age">Age :</label>
          <p  id="age"><?php echo $_SESSION['faculty_ag']; ?></p>
        </div>
        <hr>
        <div class="detail" >
          <label  for="sex">Sex :</label>
          <p  id="sex" ><?php echo $_SESSION['faculty_sx']; ?></p>
        </div>
        <hr>
         <div class="detail" >
          <label for="academic" >Position :</label>
          <p id="academic" ><?php echo $_SESSION['faculty_position']; ?></p>
         </div>
      </div>
      <div class="profile-picture-container" >
          <div class="profile-picture-bg" >
          <?php
             $sql="SELECT * FROM staff WHERE username='".$_SESSION['faculty_u_name']."' ";
              $res=mysqli_query($conn,$sql) or die( mysqli_error($conn));
              $rows=mysqli_num_rows($res);
              if($rows>0){
                while ($rows=mysqli_fetch_assoc($res)){
           ...
           <div class="changepw-buttons">
              ...
              <?php
                  if(isset($_POST["change_pw"])){
                    ...
                    $sql_change_password=mysqli_query($conn,"SELECT * FROM login WHERE username='".$_SESSION['faculty_u_name']."'") or die(mysqli_error($conn));
                    ...
                    while ($rows=mysqli_fetch_assoc($sql_change_password)){
                        ...
                        if($new_password==$confirm_password && $old_password == $old_pw){
                          $sql_change=mysqli_query($conn,"UPDATE login SET password='$new_password' WHERE username='".$_SESSION['faculty_u_name']."' ") or die(mysqli_error($conn));
...
<!-- INCLUDE FOOTER -->
    <footer>
   Copyright ©2024 MorrisBenardSila
    </footer>
